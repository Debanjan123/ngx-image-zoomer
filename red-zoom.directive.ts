...
export class RedZoomDirective implements AfterContentInit, OnChanges, OnDestroy {
    ...
    @Input('srca') @HostBinding('attr.srca') set srca(val){
        this.src =  val.changingThisBreaksApplicationSecurity
    }
    src = ''
    lensSrc = ''
    @Input('redZoom') set lensSrca(val){
        this.lensSrc = val.changingThisBreaksApplicationSecurity
    }
